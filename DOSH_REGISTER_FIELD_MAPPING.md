...
**Row unit: one row per linked `Substances` record**, not one row per `Chemicals` record (v2.1 — a rendering/aggregation decision, not a data model change; `Chemicals` remains the master table, `Substances` remains a linked sub-table, table ID `tblCirtjLUiBij9Rz`/`tblKbPw4tgcOuLri7`). A Chemical with zero linked Substances still emits exactly one row (blank Name of Chemical/CAS No.) so it's never silently missing from the register.
...
| Quantity | `Chemicals` | `quantity` → `fldT4nF1jJvpPBUhu` | Existing | Chemical-level, repeats per row. Display label elsewhere is "Current Quantity"; field ID unchanged. |
| **Type#** | `Chemicals` | `typeOfUse` → `fldadV7JEC8sRDaan` | Existing | **Derived, not a new field.** Per the guideline's own Appendix 1 coding: Raw Material→R, Product→P, By-product→B, Intermediate-product→I, Stored→S, Waste→W, Cleaning→C, Degreasing→D, Other→O. Chemical-level, repeats per row. |
| **Class** | `Chemicals` | `hazardClassification` → `fldrkm7F45ofeceuj` | Existing | **Derived, not a new field.** Shows the value verbatim, or `"NA"` if unset — matching the guideline's own instruction ("if classified using other classification system, please enter NA"). Chemical-level, repeats per row. |
...
| Workers Exposed | `Chemicals` | `workersExposed` → `fldRM4dI54znwR04p` | Existing | Chemical-level, repeats per row; `""` renders as `—`. |
| Usage of Chemical | `Chemicals` | `typeOfUse` → `fldadV7JEC8sRDaan` | Existing | Same underlying field as Type# above, shown here as its full text (e.g. "Raw Material") alongside the coded column — not a duplicate field, just two display representations of one value. |
| Engineering Control | `Chemicals` | `controlMeasures` → `fldi95R3Fz8Hn7Mjj` | Existing | Free text, kept as-is rather than attempting to auto-derive the guideline's E/I/LEV/GV/W letter codes from unstructured text — see Known Limitations. |
| PPE | `Chemicals` | `ppeActuallyUsed` → `fldZJybnxiaO1ri4V` | Existing | Free text, same reasoning as Engineering Control (guideline codes: C/E/M/R/O). Distinct from `ppeRequirement`, which this report does not use. |

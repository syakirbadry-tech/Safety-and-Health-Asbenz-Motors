...
3h. **Company Profile as central administration — done.** Expanded Company Profile into the single admin surface for everything a generated report's appearance depends on: 28 new fields (branding colours/watermark, document numbering/footer/paper defaults, a third Approved By signatory with optional embedded signature images, 10 export-setting show/hide toggles), a reusable drag-and-drop upload component, dirty-state tracking, and a live "Preview Report" that renders through the same `ReportEngine` calls a real report uses. `ReportEngine`'s primitives became company-aware so reports no longer pass individual show/hide flags through — DOSH's government-form exceptions (no logo/stamp/colour) are the one explicit opt-out. See ARCHITECTURE.md §5.7, COMPANY_PROFILE_IMPLEMENTATION_REPORT.md.
3i. **Chemical Process Usage, SDS duplicate detection, DOSH completion tracking — done.** Closed the genuine gaps a compliance audit found against the real DOSH guideline PDF (a resubmitted external design draft's claims were checked again and not applied — see DOSH_CHEMICAL_REGISTER_COMPLIANCE_REPORT.md). New `Chemical Process Usage` table lets a chemical be used across multiple processes/locations, each with its own quantity/workers-exposed/controls/PPE/type-of-use (additive — existing flat fields remain the fallback, backfilled non-destructively); SDS import duplicate detection (View Existing/Update Existing/Create New Anyway) before the wizard saves; a shared DOSH completion scorer surfaced as a Profile badge, a Register "Requires Attention" filter, and dashboard KPI cards; "Print by Process"/"Print This Chemical" scoped report views, which needed the app's router to gain its first query-string support. See ARCHITECTURE.md §5.2.5, DATABASE.md §3.4.5, DOSH_CHEMICAL_REGISTER_COMPLIANCE_REPORT.md.

...
SDS Library is a view over `Chemicals.SDS`, not a separate table. Emergency Spill Procedure is a filtered view of `SOP` records where the new `Document Type` field (§3.7) equals "Spill Response" — also not a separate table.

`GET /api/chemicals/:id/profile` aggregates a chemical with all of the above (plus linked CHRA) in one call, built from the same generic `server/lib/profileAggregation.js` helper the Machinery profile endpoint uses (see §3.3.1 for why filtering happens in application code, not `filterByFormula`).
